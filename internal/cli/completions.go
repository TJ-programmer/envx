...
const bashCompletion = `_envx_completions() {
    ...
    local commands="init set get list unset run env import export diff doctor config completions help"
...
_arguments '1:command:(init set get list unset run env import export diff doctor config completions help)' '*::arg:->args'
`

const fishCompletion = `complete -c envx -f -a "init set get list unset run env import export diff doctor config completions help"
...
const powershellCompletion = `Register-ArgumentCompleter -Native -CommandName envx -ScriptBlock {
    ...
    'init','set','get','list','unset','run','env','import','export','diff','doctor','config','completions','help' |
